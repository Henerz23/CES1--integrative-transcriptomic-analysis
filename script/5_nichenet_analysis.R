...
run_nichenet <- function(lr_network_use,
  ...
  # Find DE genes: N vs T in T cells
...
##############################
# run nichenet function for all 6 dataframes x2
##############################
...
for (net_name in names(lr_networks)) {
  ...
  for (lfc in c(0.25, -0.25)) {
    ...
    results[[run_name]] <- run_nichenet(
      ...
      logfc_cutoff = lfc,
      ngene_targets_use = 300, # top 300 genes for later figure
      top_n = 31 # use all ligands from cellchat 
      # (the highest number of ligands in the lr_networks is 31)
    )
  }
}
...
# only upregulated genes analysed
...
mat_list <- list(
  ...
  # lr_c1qc_TCD8 = results$lr_c1qc_TCD8_up$gene_expression, # excluded as no ligands present
