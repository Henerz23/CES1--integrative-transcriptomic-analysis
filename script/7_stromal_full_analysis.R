...
# Part 1: Clustering and CES1 analysis
# Part 2: Cellchat
# Part 3: Nichenet
...
## normalization
pbmc_stromal <- NormalizeData(pbmc_stromal, normalization.method = "LogNormalize", scale.factor = 10000) 
pbmc_stromal <- FindVariableFeatures(pbmc_stromal, selection.method = 'vst', nfeatures = 2000)

# scaling
pbmc_stromal <- ScaleData(pbmc_stromal, vars.to.regress = "percent.mt")
pbmc_stromal <- RunPCA(pbmc_stromal, features = VariableFeatures(object = pbmc_stromal)) 

# dim use identification
ElbowPlot(pbmc_stromal, reduction = "pca", ndims = 50)

# dimensional reduction
dim.use<-1:20
pbmc_stromal <- FindNeighbors(pbmc_stromal, dims = dim.use)
pbmc_stromal <- FindClusters(pbmc_stromal, resolution = 0.8 )
...
pbmc_stromal.markers <- FindAllMarkers(pbmc_stromal, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
pbmc_stromal.markers %>%
  group_by(cluster) %>%
  dplyr::filter(avg_log2FC > 1)

# saveRDS(pbmc_stromal.markers, file = "../data/pbmc_stromal.markers.rds")
# pbmc_stromal.markers <- readRDS("../data/pbmc_stromal.markers.rds")
...
DimPlot(object = pbmc_stromal, group.by="orig.ident", reduction='umap')

# metastasis status
# one group is called:
# pM1c (Metastases the peritoneal surface, alone or with other site or organ metastases): Sites involved: Liver and peritoneum.
# change to pM1c
pbmc_stromal@meta.data$MetastasisStatus[pbmc_stromal@meta.data$MetastasisStatus == 
...
DimPlot(object = pbmc_stromal, group.by="MetastasisStatus",reduction='umap')

# tissue site
DimPlot(object = pbmc_stromal, group.by="TissueSiteSimple",reduction='umap')

# sex
DimPlot(object = pbmc_stromal, group.by="Sex",reduction='umap')

# N or T
DimPlot(object = pbmc_stromal, group.by="SPECIMEN_TYPE",
              reduction='umap', cols = distinctColorPalette(4))

# mmrstatus
DimPlot(object = pbmc_stromal, group.by="MMRStatus",reduction='umap')
...
DotPlot(pbmc_stromal, features = 'CES1', cols = cont_2, group.by = "clMidwayPr") + RotatedAxis()
# fibro stem cell niche is high but these are alll normal cells so make CAF only calss

# dotplot of just the CAFs
pbmc_CAF <- subset(pbmc_stromal, cl295v11SubFull %in% c("CXCL14+ CAF", "GREM1+ CAF", "MMP3+ CAF", "Myofibro"))
...
# make a box plot of average CES1 expression for each patient and group by clMidwayPr
df_avg_CES1_exp_f <-  pbmc_stromal@meta.data %>%
  # group by
  group_by(orig.ident, clMidwayPr) %>%
...
exp_pct_S_C_f <-  pbmc_stromal@meta.data %>%
  ##group by
  group_by(orig.ident, clMidwayPr) %>%
  ##the summarise function
...
# Extract enriched L-R pairs for your signaling pathways of interest
...
run_nichenet <- function(lr_network_use,
  ...
  # Find DE genes: N vs T Fibroblast Cells
  ...
  geneset_oi <- if (logfc_cutoff > 0) {
    DE_table_receiver %>% filter(p_val_adj <= 0.05, avg_log2FC >= logfc_cutoff) %>% rownames()
  } else {
    DE_table_receiver %>% filter(p_val_adj <= 0.05, avg_log2FC <= logfc_cutoff) %>% rownames()
  }
...
##############################
# run nichenet function for all 3 dataframes x2
##############################
...
for (net_name in names(lr_networks_Fibro)) {
  ...
  for (lfc in c(0.25, -0.25)) {
    ...
    results[[run_name]] <- run_nichenet(
      ...
      top_n = 31 # use all ligands from cellchat (31 is the max)
    )
  }
}
